...
fn test_semantic_search_with_different_words() {
    ...
    let results = system
        .search("pizza Friday", &Default::default(), 5)
        .unwrap();
    assert!(
        !results.is_empty(),
        "Should find results for exact keywords"
    );
    let top_result = &results[0];
    assert!(
        top_result.memory.text.contains("pizza"),
        "Top result should contain 'pizza'"
    );
    println!(
        "Test 1 - Exact keywords: similarity={:.3}",
        top_result.similarity_score
    );

    // Test 2: Semantic search with different words (same concept)
    let results = system
        .search("weekend Italian food preferences", &Default::default(), 5)
        .unwrap();
    assert!(
        !results.is_empty(),
        "Should find results for semantic query"
    );
    ...
    let results = system
        .search("machine learning algorithms", &Default::default(), 5)
        .unwrap();
    if !results.is_empty() {
        let top_similarity = results[0].similarity_score;
        println!(
            "Test 3 - Unrelated query: top similarity={:.3}",
            top_similarity
        );
