...
async fn benchmark_full_pipeline_end_to_end() -> Result<()> {
    ...
    let avg_episode =
        episode_times.iter().sum::<std::time::Duration>() / episode_times.len() as u32;
    ...
    for i in 0..50 {
        let memory = agent_memory_rs::storage::Memory {
            ...
            agent_id: None,
            text: format!(
                "Important knowledge item {}: Rust is a systems programming language",
                i
            ),
    ...
    let avg_semantic =
        semantic_times.iter().sum::<std::time::Duration>() / semantic_times.len() as u32;
    ...
    let avg_hierarchical =
        hierarchical_times.iter().sum::<std::time::Duration>() / hierarchical_times.len() as u32;
    ...
    println!("═══════════════════════════════════════════");
    println!(
        "Episode Storage:        {:?} ({:.1}/sec)",
        avg_episode,
        1.0 / avg_episode.as_secs_f64()
    );
    println!(
        "Semantic Storage:       {:?} ({:.1}/sec)",
        avg_semantic,
        1.0 / avg_semantic.as_secs_f64()
    );
    println!(
    ...
    // Assertions
    assert!(
        avg_episode.as_millis() < 100,
        "Episode storage should be < 100ms"
    );
    assert!(
        avg_hierarchical.as_millis() < 10,
        "Hierarchical search should be < 10ms"
    );
