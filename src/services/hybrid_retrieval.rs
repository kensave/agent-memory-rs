...
impl HybridRetrievalEngine {
    ...
    pub fn hybrid_search(
        ...
        let vector_results = if let Some(ref embedder) = self.embedder {
            ...
            let embedding_blob: Vec<u8> = query_emb.iter().flat_map(|&f| f.to_le_bytes()).collect();
            ...
            self.db.execute(|conn| {
                ...
                let rows = stmt.query_map(
                    rusqlite::params![embedding_blob, workspace_id, limit * 2],
                    |row| {
        ...
        for result in bm25_results.into_iter().chain(vector_results) {
            all_results.entry(result.id).or_insert(result);
        }

        // Apply fused scores
        let mut final_results: Vec<HybridSearchResult> = all_results
            .into_iter()
            .map(|(id, mut r)| {
