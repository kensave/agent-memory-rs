...
impl FastEmbedder {
    ...
    pub fn load_model(&mut self) -> Result<()> {
        ...
        let vb =
            unsafe { VarBuilder::from_mmaped_safetensors(&[&model_path], DTYPE, &self.device)? };
    ...
    fn create_batch_tensors_direct(
        &self,
        ...
        for (i, encoding) in encodings.iter().enumerate() {
            ...
            token_ids[offset..offset + encoding.get_ids().len()]
                .copy_from_slice(encoding.get_ids());
            attention_mask[offset..offset + encoding.get_attention_mask().len()]
                .copy_from_slice(encoding.get_attention_mask());
        }
